...
export default function ReorderPage() {
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl p-4" style={{ background: '#ef444418', border: '1px solid #ef4444' }}>
        <div className="flex items-center gap-2">
          <AlertTriangle size={18} style={{ color: '#ef4444' }} />
          <span className="text-sm font-semibold" style={{ color: '#ef4444' }}>{lowStockItems.length} items need reordering</span>
        </div>
      </div>
            ...
            <tbody>
              {lowStockItems.map(item => (
                <tr key={item.id} className="transition" style={{ borderBottom: '1px solid var(--border)' }}
                  ...
                  <td className="px-5 py-4">
                    <span className="px-2.5 py-1 rounded-full text-xs font-semibold" style={{ background: '#ef444418', color: '#ef4444' }}>
